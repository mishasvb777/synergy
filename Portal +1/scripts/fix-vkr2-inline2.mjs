...
if (!fs.existsSync(expansionsPath)) throw new Error('expansions json missing');
...
foreach ($exp in $expansions) {
  ...
  # Find last substantial body paragraph before caption
  $best = $null
  foreach ($para in @($doc.Paragraphs)) {
    if ($para.Range.Start -lt $assignGuardEnd) { continue }
    if ($para.Range.End -ge $cap.Start) { break }
    if ($para.Range.InlineShapes.Count -gt 0) { continue }
    $t = $para.Range.Text
  ...
  $insertRange = $best.Range.Duplicate
  ...
  Write-Output ('OK ' + $mark + ' after pos ' + $best.Range.Start)
}
...
fs.writeFileSync(path.join(diploma, '_extract/fix_vkr2_inline2.ps1'), '\uFEFF' + ps, 'utf8');
const r = spawnSync(
  'powershell',
  ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', path.join(diploma, '_extract/fix_vkr2_inline2.ps1')],
