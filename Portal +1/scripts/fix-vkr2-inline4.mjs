...
# Real body anchor: Актуальность is unique to introduction text
$act = Find-First 'Актуальность темы обусловлена'
if ($null -eq $act) { throw 'No Актуальность' }

# Body ВВЕДЕНИЕ = last short heading "ВВЕДЕНИЕ" before Актуальность
$bodyIntro = $null
$cand = Find-First 'ВВЕДЕНИЕ'
while ($null -ne $cand -and $cand.Start -lt $act.Start) {
  $paraText = $cand.Paragraphs.Item(1).Range.Text
  $plain = $paraText.Trim().TrimEnd([char]13, [char]7, [char]11)
  if ($plain -eq 'ВВЕДЕНИЕ' -or $plain.StartsWith('ВВЕДЕНИЕ')) {
    if ($plain.Length -le 40) { $bodyIntro = $cand.Duplicate }
  }
  $cand = Find-From $cand.End 'ВВЕДЕНИЕ'
}
if ($null -eq $bodyIntro) { throw 'No body ВВЕДЕНИЕ' }

# Real TOC heading = last "СОДЕРЖАНИЕ" before body ВВЕДЕНИЕ
$tocH = $null
$cand = Find-First 'СОДЕРЖАНИЕ'
while ($null -ne $cand -and $cand.Start -lt $bodyIntro.Start) {
  $paraText = $cand.Paragraphs.Item(1).Range.Text
  $plain = $paraText.Trim().TrimEnd([char]13, [char]7, [char]11)
  if ($plain -eq 'СОДЕРЖАНИЕ' -or ($plain.StartsWith('СОДЕРЖАНИЕ') -and $plain.Length -le 40)) {
    $tocH = $cand.Duplicate
  }
  $cand = Find-From $cand.End 'СОДЕРЖАНИЕ'
}
if ($null -eq $tocH) { throw 'No real СОДЕРЖАНИЕ' }

$assignGuardEnd = $tocH.Start
Write-Output ('Real TOC at ' + $tocH.Start + ' bodyIntro ' + $bodyIntro.Start + ' act ' + $act.Start)
Write-Output ('Assignment zone 0..' + $assignGuardEnd + ' (untouched)')

# Snapshot assignment hash-ish length to verify unchanged later
$assignLen0 = $doc.Range(0, $assignGuardEnd).Text.Length
...
foreach ($exp in $expansions) {
  ...
  $cap = Find-From $assignGuardEnd $needle
  if ($null -eq $cap -and $needle.Length -gt 34) { $cap = Find-From $assignGuardEnd $needle.Substring(0, 34) }
  if ($null -eq $cap) { Write-Output ('MISS ' + $needle); continue }

  $best = $null
  $imageBeforeCap = $false
  foreach ($para in @($doc.Paragraphs)) {
    if ($para.Range.Start -lt $assignGuardEnd) { continue }
    if ($para.Range.End -ge $cap.Start) { break }
    if ($para.Range.InlineShapes.Count -gt 0) { $imageBeforeCap = $true; continue }
    $t = $para.Range.Text
  ...
  $cap2 = Find-From $assignGuardEnd $needle
  if ($null -eq $cap2 -and $needle.Length -gt 34) { $cap2 = Find-From $assignGuardEnd $needle.Substring(0, 34) }
  $fmtEnd = if ($null -ne $cap2) { $cap2.Start } else { $anchorEnd + $block.Length + 10 }
  $fmt = $doc.Range($anchorEnd, $fmtEnd)
  ...
  $phrase = [string]$exp.paragraphs[0]
  if ($phrase.Length -gt 40) { $phrase = $phrase.Substring(0, 40) }
  $ph = Find-From $assignGuardEnd $phrase
  $cap3 = Find-From $assignGuardEnd $needle
  if ($null -eq $cap3 -and $needle.Length -gt 34) { $cap3 = Find-From $assignGuardEnd $needle.Substring(0, 34) }
  $ok = ($null -ne $ph -and $null -ne $cap3 -and $ph.Start -lt $cap3.Start)
  Write-Output ('OK ' + $mark + ' phraseBeforeCap=' + $ok + ' img=' + $imageBeforeCap)
}
Write-Output ('InsertedParas=' + $inserted)
if ($inserted -lt 8) { throw 'Too few insertions' }

# Re-resolve anchors after insertions
$act = Find-First 'Актуальность темы обусловлена'
$bodyIntro = $null
$cand = Find-First 'ВВЕДЕНИЕ'
while ($null -ne $cand -and $cand.Start -lt $act.Start) {
  $plain = $cand.Paragraphs.Item(1).Range.Text.Trim().TrimEnd([char]13, [char]7, [char]11)
  if (($plain -eq 'ВВЕДЕНИЕ' -or $plain.StartsWith('ВВЕДЕНИЕ')) -and $plain.Length -le 40) { $bodyIntro = $cand.Duplicate }
  $cand = Find-From $cand.End 'ВВЕДЕНИЕ'
}
$tocH = $null
$cand = Find-First 'СОДЕРЖАНИЕ'
while ($null -ne $cand -and $cand.Start -lt $bodyIntro.Start) {
  $plain = $cand.Paragraphs.Item(1).Range.Text.Trim().TrimEnd([char]13, [char]7, [char]11)
  if (($plain -eq 'СОДЕРЖАНИЕ' -or $plain.StartsWith('СОДЕРЖАНИЕ')) -and $plain.Length -le 40) { $tocH = $cand.Duplicate }
  $cand = Find-From $cand.End 'СОДЕРЖАНИЕ'
}
$assignGuardEnd = $tocH.Start
$assignLen1 = $doc.Range(0, $assignGuardEnd).Text.Length
Write-Output ('AssignLen before=' + $assignLen0 + ' afterExp=' + $assignLen1)
if ($assignLen1 -ne $assignLen0) { throw 'Assignment length changed during expansions' }

# Rebuild ONLY the real TOC (between real СОДЕРЖАНИЕ and body ВВЕДЕНИЕ)
$doc.Range($tocH.End, $bodyIntro.Start).Delete() | Out-Null
...
foreach ($it in $items) {
  ...
  # Page from bookmark location via GoTo if possible
  try {
    $br = $doc.Bookmarks.Item($bm).Range
    $pages[$label] = [int]$br.Information(3)  # wdActiveEndPageNumber
  } catch {
...
# Re-resolve and format TOC only
$act = Find-First 'Актуальность темы обусловлена'
$bodyIntro = $null
$cand = Find-First 'ВВЕДЕНИЕ'
while ($null -ne $cand -and $cand.Start -lt $act.Start) {
...
$tocBody = $doc.Range($tocH.End, $bodyIntro.Start)
...
$assignLen2 = $doc.Range(0, $tocH.Start).Text.Length
Write-Output ('AssignLen final=' + $assignLen2)
if ($assignLen2 -ne $assignLen0) { throw 'Assignment changed after TOC rebuild' }

$tocText = $tocBody.Text
if ($tocText -match 'Руководитель' -or $tocText -match 'библиографическ') { throw 'TOC still contains assignment text' }
if ($tocText -notmatch 'ГЛАВА 1') { throw 'TOC missing chapters' }

Write-Output ('CONCLUSION_PAGE ' + $pages['ЗАКЛЮЧЕНИЕ'])
Write-Output ('TOC_PREVIEW ' + $tocText.Substring(0, [Math]::Min(200, $tocText.Length)))
...
fs.writeFileSync(path.join(diploma, '_extract/fix_vkr2_inline4.ps1'), '\uFEFF' + ps, 'utf8');
const r = spawnSync(
  'powershell',
  ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', path.join(diploma, '_extract/fix_vkr2_inline4.ps1')],
